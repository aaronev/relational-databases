...
SELECT ROUND(AVG(fertility_rate)) AS average_fertility_rate_of_all_countries
  FROM countries_by_population;
SELECT country, MAX(percent_aged_over_60) AS country_with_most_alive_over_60
  FROM countries_by_population;
SELECT country, MIN(urban_pop) AS smallest_urban_population
  FROM countries_by_population;

SELECT country, fertility_rate, urban_pop
  FROM countries_by_population
  GROUP BY fertility_rate
  HAVING urban_pop_percent < 20;

SELECT country, percent_aged_over_60, fertility_rate
  FROM countries_by_population
  WHERE percent_aged_over_60 > 11.41
